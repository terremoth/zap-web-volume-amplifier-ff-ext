const intervalAudioAmp = setInterval(() => {
    ...
    if (header) {
        ...
        function handleVolumeRateStatus (rate) {
            ...
            if (volumeRate === 1) {
                button.innerHTML = "2x";
                button.classList.add("twoTimesButtonAmp");
                button.classList.remove("oneTimeButtonAmp");
                localStorage.setItem("volumeRate", "1");
              } else if (volumeRate === 2) {
                button.innerHTML = "1x";
                button.classList.add("oneTimeButtonAmp");
                button.classList.remove("twoTimesButtonAmp");
